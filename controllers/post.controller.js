...
const makePost = async (req, res) => {
	const { title, desc, content, tags, previewPix, detailPix, published } =
		req.body;
	...
	try {
		...
		const newPost = new postModel({
			...
			desc,
			content,
			creatorId, // Link to the user's ID from the JWT
			tags,
			previewPix,
			detailPix,
			published,
...
const getAllPost = async (req, res) => {
	try {
		...
		console.error("Error retrieving posts:", error); // Log error for debugging
		res.status(500).json({
			error: "Cannot retrieve posts. Please try again later.",
		});
	}
};
...
const updatePost = async (req, res) => {
	...
	try {
		...
		if (Object.keys(updates).length === 0) {
			return res
				.status(400)
				.json({ error: "No changes provided for update" });
		}
		...
		if (!updatedPost) {
			return res
				.status(304)
				.json({ message: "Post is already up-to-date" });
		}
		...
		res.status(500).json({
			error: "Cannot update post. Please try again later.",
		});
	}
};
...
const getPost = async (req, res) => {
	...
	try {
		...
		console.error("Error retrieving post:", error); // Log error for debugging
...
const likePost = async (req, res) => {
	...
	try {
		...
		console.error("Error processing like/unlike:", error); // Log any errors for debugging
		res.status(500).json({ error: "Error processing like/unlike" });
	}
};

// Export the post-related functions
module.exports = {
	makePost,
	getAllPost,
	getPost,
